...
class HearstPatterns(object):
    ...
    def __init__(self, extended=False):
        ...
        self.__hearst_patterns = [
                ...
                ( "(NP_\w+ (, )?(NP_\w+ ?(, )?)*or other NP_\w+)","third"),
                ( "(NP_\w+ (, )?(NP_\w+ ?(, )?)*and other NP_\w+)","fourth"),

                ("(such NP_\w+ (, )?as (NP_\w+ ? (, )?(and |or )?)+)","second"),
                ("(NP_\w+ (, )?including (NP_\w+ ? (, )?(and |or )?)+)","fifth"),
                #First word, optional ", "especially (word ", "and word ")
                ("(NP_\w+ (, )?especially (NP_\w+ ? (, )?(and |or )?)+)","sixth")
            ...
            for (hearst_pattern, parser) in self.__hearst_patterns:
                ...
                if matches:
                    ...
                    if parser == "first":
                        ...
                        hypernym = nps[0]
                        hyponyms = nps[1:]
                        if( parser == "1" or parser == "2" or parser == "3" or parser == "4" or parser == "5" or parser == "6"):
                            #print ( parser )
                            count[ int(parser) ] = 1 + count[ int(parser) ]
                            #print (parser)
                    #    #print("Parser: ",parser)
                    #    hypernym = nps[-1]
                    #    hyponyms = nps[:-1]
